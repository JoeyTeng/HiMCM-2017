    ...
    while updated:
        ...
        for i in range(origin_img.shape[0]):
            for j in range(origin_img.shape[1]):
                if origin_img[i][j][origin_height] == DefaultColor:
                    prev = (i, j, origin_height)
                    origin_img[i][j][origin_height] = BackgroundColor
                    break
            if prev is not None:
                break
        ...
        for i in range(target_img.shape[0]):
            for j in range(target_img.shape[1]):
                if target_img[i][j][target_height] == DefaultColor:
                    p.append(((prev[0], prev[1], prev[2]),
                              (i, j, target_height), ))

                    target_img[i][j][target_height] = BackgroundColor
                    updated = True
                    break
            if updated:
                break
...
for vertex in origin_vertices:
    origin[vertex[0]][vertex[1]][origin_height] = DefaultColor
for vertex in target_vertices:
    target[vertex[0]][vertex[1]][target_height] = DefaultColor
